namespace Bede.Lottery
{
    ...
    /// </summary>
    [ExcludeFromCodeCoverage(Justification = "Code is used in testing.")]
    internal static class MoqVerifyAsyncExtensions
    {
        ...
        public async static Task SomeVerifyWithTimeout(Action action, TimeSpan timeout)
        {
            ...
            while (stopWatch.Elapsed < timeout)
            {
                ...
                catch (MockException mockException)
                {
                    ...
                    await Task.Delay(GRANULARITY).ConfigureAwait(false);
                }
            }
        ...
        public async static Task VerifyAsync<T, TExpressionResult>(this Mock<T> mock, Expression<Func<T, TExpressionResult>> expression, Times times, TimeSpan timeout) where T : class
        {
            await SomeVerifyWithTimeout(() => mock.Verify(expression, times), timeout).ConfigureAwait(false);
        }
        ...
        /// <param name="timeout">The timeout within the verify should be successful.</param>
        /// <returns>A task to wait on. The task will fail with a TimeoutException in case the Verify() was not successful within the given timeout.</returns>
        public async static Task VerifyAsync<T>(this Mock<T> mock, Expression<Action<T>> expression, Times times, TimeSpan timeout) where T : class
        {
            await SomeVerifyWithTimeout(() => mock.Verify(expression, times), timeout).ConfigureAwait(false);
        }
    }
